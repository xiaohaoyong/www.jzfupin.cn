...
class content extends Admin {
	...
	public function addAction() {
	    ...
	    if ($this->post('submit')) {
			...
			if (!empty($data['seourl'])) {
				if($this->check_seourlAction($data['seourl'])) $this->show_message('seourl已经存在',2,1);
			};
			...
			$data['authorid'] = $this->admin['id'];//记录运营id
    ...
    public function editAction() {
	    ...
	    if ($this->post('submit')) {
			...
			if (!empty($data['seourl'])) {
				if($this->check_seourlAction($data['seourl'],$id)) $this->show_message('seourl已经存在',2,1);
			};
	        ...
	        $data['username'] = $this->admin['username'];//记录运营username
	        $data['authorid'] = $this->admin['id'];//记录运营id
	 ...
	 * 判断seourl是否重复
	 */
    private function check_seourlAction($seourl,$id=0) {
		if($this->db->setTableName('content')->getAll(array('id<>?', 'seourl=?'), array($id, $seourl)))
		return true;
	    else
		return false;
	}
	...
	private function additionalAction($data) {
		...
		if (isset($data['content']) &&  $data['xiao_download_image'] && $this->site_config['site_download_image']) {
		    if (preg_match_all("/(src)=([\"|']?)([^ \"'>]+\.(gif|jpg|jpeg|bmp|png))\\2/i", $content, $imgs)) {
				...
				$fileext =  fileext($img);
				$name	 = $path . substr(md5($img . time()),8,16) . '.' . $fileext;
				$content = @file_get_contents($img);
				if (empty($content)) continue;
			  	if (file_put_contents(IAEWEB_PATH .$name, $content)) {
				    if ($this->site_config['site_watermark']) $image->watermark(IAEWEB_PATH . $name,$this->site_config['site_watermark_pos']);
				}
				$regex[]   = $img;
