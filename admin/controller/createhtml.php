...
class createhtml extends Admin {
    ...
    public function __construct() {
        parent::__construct();
		if ($this->site_config['diy_url'] != 2) $this->show_message('请开启系统生成静态功能',2,  url('index/config',array('type'=>4)));
		$this->tree =  iaeweb::load_class('tree');
	}
	...
	public function indexAction() {
		ob_start();
	    iaeweb::load_file(IAEWEB_PATH .'core/controller/index.php');
	    $app      = new index();
		$app->indexAction();
		if (!file_put_contents(IAEWEB_PATH.'index.html', ob_get_clean(), LOCK_EX)) $this->show_message($url.'生成失败！',2,'');
	...
	protected function create_category_html($cat, $page=0) {
		$cat['catdirs'] = get_parent_dir($cat['catid']); //全目录
        $cat['page'] = $page;
        if (!empty($page))
	    ...
	    if (substr($url, -5) != '.html') { 
	    	mkdirs(IAEWEB_PATH.$url);
	    	$htmlfile  = IAEWEB_PATH . $url . '/' . 'index.html';
		} else {
 	    	mkdirs(IAEWEB_PATH.dirname($url));
    		$htmlfile  = IAEWEB_PATH . $url;
		}
		ob_start();
	...
	protected function create_show_html($content, $page=0) {
		//获取url
		$content['catdirs'] = get_parent_dir($content['catid']); //全目录
