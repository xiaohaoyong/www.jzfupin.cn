...
class index extends Admin {
	...
	public function mainAction() {
		...
		$data = http_build_query(array('domain'=> $_SERVER['HTTP_HOST'],'version'=>'x1','release'=>IAEWEB_RELEASE,'time'=> date("Y-m-d")));
		$client_url = 'http://cms.iaeweb.com/index.php?c=index&a=license&'.$data;
	...
	public function configAction() {
        ...
        if ($this->post('submit')) {
			...
			$config_txt = "<?php" . PHP_EOL . "if (!defined('IN_IAEWEB')) exit();" . PHP_EOL . "return " . $config_arr. ";";
	...
	public function cacheAction() {
    	$modules = array(
			...
			4   => array('内链缓存更新成功..........',  'diytable',       'cache&modelid=6'),
			5   => array('采集缓存更新成功..........',  'spider',       'cache'),
			6   => array('滤镜缓存更新成功..........',  'tool',       'cache'),
	    );
	    if ($this->get('show')) {
	        ...
	        $id ++;
			if (!empty($m)) {
				echo '<script type="text/javascript">window.parent.frames["hidden"].location="index.php?c='. $c .'&a='.$m[2].'";</script>';
