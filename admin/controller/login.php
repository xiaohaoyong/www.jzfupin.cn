    ...
    public function indexAction() {
		if ($this->post('submit')) {
		    ...
		    if ($admin['username'] == $username &&  $admin['password'] == md5(md5($password))) {
			    ...
			    $this->show_message('恭喜您！'.$username.' 登录成功', 1, base64_decode('Li8/aWFld2Vi'));
		    } else {
			    if ($this->session->get('admin_login_error_num')) {
		...
		if ($this->session->get('user_id')) $this->show_message('你已登陆系统', 1, url('index'));
        include $this->admin_tpl('login');
    }
    
    public function logoutAction() {
        if ($this->session->get('user_id')) $this->session->delete('user_id');
        $this->show_message('已经成功退出iAEweb系统', 1, url('login'));
    }
