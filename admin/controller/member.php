...
class member extends Admin {
    ...
    public function __construct() {
		parent::__construct();
		if (!defined('IAEWEB_MEMBER')) $this->show_message('没有安装会员模块，请去iAEweb下载安装',2 , 'http://www.iaeweb.com');
	}
    ...
    public function indexAction() {
		...
		/*获取所有会员*/
		$data_list = $this->db->setTableName('member')->findAll(null,'id DESC');
	...
	public function editAction() {
		...
		$docs = $this->db->setTableName('member')->where('modelid=?',7)->where('status=?',1)->getAll();
		...
		if ($this->post('submit')) {
			...
			if($data['userid'] > 0 && $data['cid'] > 0) $this->db->setTableName('content')->update(array('userid'=>$data['userid']),'id=?',$data['cid']);//更改居民资料
	...
	public function delAction($id=0,$show=0) {
		...
		$this->db->setTableName("member_connect")->delete('uid=?' , $id);// 增加删除 微信
		$path = IAEWEB_PATH . 'data/upload/member/' . $id . '/';
