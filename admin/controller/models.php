...
class models extends Admin {
	...
	public function addAction() {
	    if ($this->post('submit')) {
			...
			$this->db->execute($sql);
		    $this->db->execute("INSERT INTO `#iaeweb_model_field` (fieldid,modelid,field,name,formtype,isshow) VALUES (NULL, $modelid,'content','内容 ','editor',1)");
    ...
    public function editAction() {
        ...
        $list_code = '
{list table=' . $data['tablename'] . '   num=10}
表单字段信息 例如：id：{$v[\'id\']} 更多信息请参考iAEweb官方模板帮助文档
{/list}';
        if ($join) {
		    $list_code = '
{list table=' . $data['tablename'] . ' cid=被关联的文章id(例如：$id) num=10}
表单字段信息 例如：id：{$v[\'id\']} 更多信息请参考iAEweb官方模板帮助文档
{/list}';
        ...
        $list_code = '
{list table=' . $data['tablename'] . '   num=10}
在这里可以调用你的diy字段信息 例如：id：{$v[\'id\']} 更多信息请参考iAEweb官方模板帮助文档
{/list}';
	    }
	...
	public function delAction() {
		...
		$this->db->execute('DELETE FROM `#iaeweb_model_field` where modelid=' . $modelid);
		$this->cacheAction();
	...
	public function addfieldAction() {
	    ...
	    if ($this->post('submit')) {
			...
			switch($data['formtype']) {
				...
				case 'checkbox':
				case 'related':
					$this->db->execute("ALTER TABLE `#iaeweb_$model_data[tablename]` ADD `$field` VARCHAR( 255 ) NOT NULL");
				break;
				case 'diy':
				case 'files':
				case 'editor':
				case 'textarea':
					$this->db->execute("ALTER TABLE `#iaeweb_$model_data[tablename]` ADD `$field` MEDIUMTEXT NOT NULL");
				break;
			}
	...
	public function delfieldAction() {
		...
		$this->db->execute("ALTER TABLE `#iaeweb_$modeldata[tablename]` DROP `$data[field]`");
		$this->cacheAction();
        $this->show_message('删除成功', 1);
	}
