...
class uploadfile extends Admin {
	...
	public function kindeditor_filemanagerAction() {
		$root_path = IAEWEB_PATH . $this->dir;
	...
	public function managerAction() {
        ...
        $file_list = glob(IAEWEB_PATH.$this->dir . $dir.'*');
		$data =$list = array();
        ...
        if ($data) {
            foreach ($data as $t) {
                ...
                $path = $dir . $t . '/';
				if (is_dir(IAEWEB_PATH . $this->dir . $path))
				{
                   $dirlist[] = array(
                    'name'     => $t, 
    ...
    private function upload($fields, $type, $size) {
		$upload   = iaeweb::load_class('upload');
		...
		$filename = substr(md5(time() . $_FILES[$fields]['name']),8,16) . '.' . $ext;
		$filenpath = $path.$filename;
        $result   = $upload->set_limit_size(1024*1024*$size)->set_limit_type($type)->upload($_FILES[$fields],IAEWEB_PATH.$filenpath);
        if (in_array($ext, array('jpg', 'gif', 'png', 'bmp'))) {
		$this->watermark(IAEWEB_PATH.$filenpath);
        }
