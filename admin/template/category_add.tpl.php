		...
		<input type="hidden" value="<?php echo $catid; ?>" name="catid">
				<div class="col-tab">
					...
					<tr class="type_3" style="display:none;">
						...
						<td><input type="text" class="input-text" size="50" value="<?php echo $data['http']; ?>" name="data[http]"><div class="onShow">请使用http://开头 如：http://cms.iaeweb.com</div></td>
					</tr>
					</tbody>
					</table>
					...
					<tr>
						...
						<input name="data[ispost]" type="radio" value="1"<?php if ($data['ispost']==1) { ?> checked<?php } ?> >&nbsp;允许游客投稿
							&nbsp;&nbsp;&nbsp;
						<?php if (!defined('IAEWEB_MEMBER')) {?><div style="display:none;"><?php } ?>
						<input name="data[ispost]" type="radio" value="2"<?php if ($data['ispost']==2) { ?> checked<?php } ?> >&nbsp;允许会员投稿 
						
						<?php foreach ($member_Model as $v) {?>
						&nbsp;&nbsp;&nbsp;
						<input name="data[ispost]" type="radio" value="<?php echo $v['modelid'];?>"<?php if ($data['ispost']==$v['modelid']) { ?> checked<?php } ?> >&nbsp;只允许 <?php echo $v['modelname'];?> 投稿
						<?php } ?>


						<?php if (!defined('IAEWEB_MEMBER')) {?></div><?php } ?>
						<div class="onShow">子栏目开放投稿 上级栏目也要开放哦</div>
						</td>
					</tr>
					<tr <?php if (!defined('IAEWEB_MEMBER')) {?>style="display:none;"<?php } ?>>
						<th >投稿审核：</th>
						<td>
					...
					<tr <?php if (!defined('IAEWEB_MEMBER')) {?>style="display:none;"<?php } ?>>
						...
						&nbsp;&nbsp;&nbsp;
						<input name="data[islook]" type="radio" value="1"<?php if ($data['islook']==1) { ?> checked<?php } ?> >&nbsp;所有会员可以查看
						<?php foreach ($member_Model as $v) {?>
						&nbsp;&nbsp;&nbsp;
						<input name="data[islook]" type="radio" value="<?php echo $v['modelid'];?>"<?php if ($data['islook']==$v['modelid']) { ?> checked<?php } ?> >&nbsp;只有 <?php echo $v['modelname'];?> 才可以查看
						<?php } ?>
						</td>
					</tr>
					<tr>
