	...
	<div class="pad-10">
		<div class="col-tab">
			...
			<table width="100%" class="table_form">
				...
				<tr>
					<th>问答字段： </th>
					<td><input class="input-text" type="text" name="data[xywy_ask]" value="<?php echo $data['xywy_ask']; ?>" size="70"/></td>
				</tr>
			</table>
			</div>
			...
			<tr>
				<th>自动内链： </th>
				<td><input name="data[site_autolink]" type="radio" value="1" <?php if (!empty($data['site_autolink'])) { ?>checked<?php } ?> /> 打开
					&nbsp;&nbsp;&nbsp;<input name="data[site_autolink]" type="radio" value="0" <?php if (empty($data['site_autolink'])) { ?>checked<?php } ?> /> 关闭
					<div class="onShow">选择打开后前台文章内容自动关键词链接</div>
				</td>
			</tr>
			</table>
			<script type="text/javascript">
			...
			<div class="contentList pad-10 hidden" id="div_setting_3" style="display: none;" >
				...
				<tr>
					<th >微信公众号登录： </th>
					<td><input name="data[wechat_login]" type="radio" value="1" <?php if ($data['wechat_login']==1) { ?>checked<?php } ?>> 打开
					&nbsp;&nbsp;&nbsp;<input name="data[wechat_login]" type="radio" value="0" <?php if ($data['wechat_login']==0) { ?>checked<?php } ?>> 关闭</td>
				</tr>
				<tr>
					<th >微信公众号appid： </th>
					<td><input name="data[appid]" class="input-text" type="text"   value="<?php echo $data['appid']; ?>"> 填写appid 请自行去微信公众号开放平台申请获取</td>
				</tr>
				<tr>
					<th >微信公众号appsecret： </th>
					<td><input name="data[appsecret]" class="input-text" type="text"   value="<?php echo $data['appsecret']; ?>" size="70"> 填写appsecret</td>
				</tr>

				<tr>
					<th >微信公众号token： </th>
					<td><input name="data[token]" class="input-text" type="text"   value="<?php echo $data['token']; ?>" size="70"> 填写token</td>
				</tr>
			...
			<div class="contentList pad-10 hidden" id="div_setting_4" style="display: none;">
					...
					<tr>
						...
						<div class="onShow">参数说明：&nbsp; {catdirs} 表示栏目路径 , {catdir} 表示栏目目录 ，{catid} 表示栏目ID</div>
						</td>
					</tr>
					<tr>
						<th>栏目URL格式(带分页)： </th>
						<td><input  class="input-text" type="text" name="data[list_page_url]" value="<?php echo $data['list_page_url']; ?>" size="40"/>
						<div class="onShow">参数说明：&nbsp;{catdirs} 表示栏目路径 , {catdir} 表示栏目目录 ，{catid} 表示栏目ID ，{page}表示分页参数</div>
						</td>
					</tr>
					<tr>
						<th>内容URL格式： </th>
						<td><input  class="input-text" type="text" name="data[show_url]" value="<?php echo $data['show_url']; ?>" size="40"/>
						<div class="onShow">参数说明：&nbsp;{catdirs} 表示栏目路径 , {catdir} 表示栏目目录 ，{id} 表示内容ID ，备注：&nbsp;{id}必须存在</div>
						</td>
					</tr>
					<tr>
						<th>内容URL格式(带分页)： </th>
						<td><input  class="input-text" type="text" name="data[show_page_url]" value="<?php echo $data['show_page_url']; ?>" size="40"/>
						<div class="onShow">参数说明：&nbsp;{catdirs} 表示栏目路径 , {catdir} 表示栏目目录 ，{id} 表示内容ID ，{page}表示分页参数 备注：&nbsp;{id}必须存在</div>
						</td>
					</tr>
					<tr>
						<th >伪静态规则在线生成：</th>
						<td><p><a target="_blank" href="http://cms.iaeweb.com/index.php?c=rewrite">在线生成伪静态规则</a></p>
						</td>
					</tr>
					<tr>
						<th >生成静态功能收费说明：</th>
						<td><p><a target="_blank" href="http://cms.iaeweb.com/buy/" style="color: #f00;">生成静态功能是收费的，购买方式请看：http://cms.iaeweb.com/buy/</a></p>
						</td>
					</tr>
