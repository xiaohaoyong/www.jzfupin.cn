...
class index extends Base {
	...
	public function formAction() {
	    ...
	    if ($this->post('submit')) {
			...
			if ($this->db->setTableName($form_model['tablename'])->insert($data,true)) {
			    ...
			    if($modelid==23) {
			    	...
			    	if($oldw){
				    	if($content_info['uid']>0 && ($data['ssy'] < 91 || $data['ssy'] > 141 || $data['szy'] < 61 || $data['szy'] > 91) && ($oldw['ssy'] < 91 || $oldw['ssy'] > 141 || $oldw['szy'] < 61 || $oldw['szy'] > 91))
				    		$this->createMassage(20,$content_info['uid'],'/?id='.$cid.'&tab=2');//通知 居民
				    }
			    }
	    ...
	    $this->view->assign(array(
			...
			'site_keywords'    => $this->site_config['site_keywords'], 
			'site_description' => $this->site_config['site_description'].' - Powered by iAEweb',
			'cid' => $cid,
	    ));
