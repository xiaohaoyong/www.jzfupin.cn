...
class install extends Base{
	public function __construct() {
		...
		if (file_exists(DATA_DIR . 'install.lock')) {
		    exit('iAEweb网站管理系统安装程序已锁定。如果要重新安装，请删除<b>/data/install.lock</b>文件！');
        }
	}
	...
	public function step3Action() {
		...
		$dbconfig_txt = "<?php" . PHP_EOL . "if (!defined('IN_IAEWEB')) exit();" . PHP_EOL . "return " . $dbconfig_arr. ";";
		if(!file_put_contents(DATA_DIR . 'config' . DIRECTORY_SEPARATOR . 'database.ini.php', $dbconfig_txt))  dexit('数据库配置文件保存失败，请检查文件权限！');
		$sql      = file_get_contents(DATA_DIR . 'install/iaeweb.sql');		
		$sql      = str_replace(array('io_', 'admin_name', 'admin_pass'), array($prefix, $admin_name, md5(md5($admin_pass))), $sql);
		$this->install_sql_execute($sql);
		if ($import) {
		$sql  = file_get_contents(DATA_DIR . 'install/iaeweb_data.sql');
		$sql      = str_replace('io_', $prefix, $sql);
		$this->install_sql_execute($sql);
		}
	...
	private function install_sql_execute($sql) {
		...
		file_put_contents(DATA_DIR . 'install.lock', 'Powered by iAEweb:'.time());
	}
	
    protected function install_tpl($file) {
        return  IAEWEB_PATH.'data/install'.DIRECTORY_SEPARATOR.$file.'.tpl.php';
    }
