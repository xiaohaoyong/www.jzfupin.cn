...
define('IN_IAEWEB', true);
error_reporting(E_ERROR | E_WARNING | E_PARSE);
$config         = iaeweb::load_config('config');
/**
 * 配置
 */
...
define('DATA_DIR',         IAEWEB_PATH . 'data' . DIRECTORY_SEPARATOR);
define('TEMPLATE_DIR',           IAEWEB_PATH . 'template' . DIRECTORY_SEPARATOR);
if (!defined('CONTROLLER_DIR')) define('CONTROLLER_DIR',     CORE_PATH . 'controller' . DIRECTORY_SEPARATOR);
define('COOKIE_PRE',			'iaeweb_');//Cookie 前缀，同一域名下安装多套系统时，请修改Cookie前缀
date_default_timezone_set('Asia/Shanghai');
iaeweb::load_file(CORE_PATH . 'library' . DIRECTORY_SEPARATOR . 'global.function.php');
iaeweb::load_file(CORE_PATH . 'version.php');
iaeweb::load_file(CORE_PATH . 'controller/Base.class.php'); 

/**
 * 系统核心全局控制类
 */
abstract class iaeweb {
	...
	public static function run() {
		...
		if (!isset($_app[$app_id])) {
			...
			if (is_file(CONTROLLER_DIR . $controller . '.php')) {	
			   ...
			   exit('iAEweb：Controller does not exist.');
			}
			$app_object = new $controller();
			if (method_exists($controller, $action)) {
				$_app[$app_id] = $app_object->$action();
			} else {
				exit('iAEweb：Action does not exist.');
			}
		}
		return $_app[$app_id];
	}
	...
	public static function load_config($file) {
		...
		$path = IAEWEB_PATH . 'data' . DIRECTORY_SEPARATOR . 'config' . DIRECTORY_SEPARATOR . $file . '.ini.php';
		if (is_file($path)) {
