...
class view
{
    ...
    protected function handle_view_file($view_content)
    {
        ...
        $replace_array = array(
            ...
            "<?php \$this->block(\\1);?>",
            "<?php \$return = \$this->_category(\"\\1\");  if (is_array(\$return))  foreach (\$return as \$key=>\$v) { \$allchildids = @explode(',', \$v['allchildids']);    \$current = in_array(\$catid, \$allchildids);?>",
            "<?php } ?>",
            "<?php \$return_\\2 = \$this->_listdata(\"\\1 return=\\2\"); extract(\$return_\\2); if (is_array(\$return_\\2))  foreach (\$return_\\2 as \$key_\\2=>\$\\2) { ?>",
            "<?php \$return = \$this->_listdata(\"\\1\"); extract(\$return); if (is_array(\$return))  foreach (\$return as \$key=>\$v) { ?>",
            "<?php } ?>",
    ...
    protected function _listdata($param)
    {
        ...
        if (is_array($paramarr)) {
            foreach ($paramarr as $key => $val) {
                if (in_array($key, array('sql', 'table', 'iaeweb', 'cache', 'page', 'urlrule', 'num', 'order', 'pagesize', 'return'))) {
                    $system[$key] = $val;
                } else {
                    $fields[$key] = $val;
                    $_fields[] = $key;
        ...
        if (!empty($system['iaeweb'])) {
            ...
            if ($table2) {
                ...
                $table2 = '#iaeweb_' . $table2;
                $from .= ' LEFT JOIN ' . $table2 . ' ON `#iaeweb_' . $table1 . '`.`id`=`' . $table2 . '`.`id`';
            }
        }
        $table1 = '#iaeweb_' . $table1;
        $where = '';
        ...
        if ($table1 == '#iaeweb_content' && !isset($fields['status'])) {
            $where .= ' AND `#iaeweb_content`.`status`!=0';
        }
        if ($where) {
    ...
    public function get_category_url($data, $page = 0)
    {
        ...
        if (!empty($this->site_config['diy_url']) && $this->site_config['list_url']) {
			$data['catdirs'] = get_parent_dir($data['catid']); //全目录
            $data['page'] = '[page]';
            if (!empty($page))
